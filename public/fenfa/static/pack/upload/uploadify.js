var xhr;var ot;var oloaded;function uploadify(){var upfile=$("#uploadify")[0].files[0];if(upfile.size>in_size*1048576){$(".uploadifyError").show().text("upload error，size not exceed "+in_size+"MB！");return false;}
...
speed=speed.toFixed(1);var per=Math.round(evt.loaded/evt.total*100);$(".uploadifyProgressBar").css("width",per+"%");$(".percentage").text(" - "+per+"% - "+speed+units);if(per>99){$(".percentage").text(" saving, Please wait...");}}
function complete(evt){var response=evt.target.responseText;return_response(response);}
function failed(){$(".uploadifySuccess").hide();$(".uploadifyError").show().text("upload error，please try！");}
function cancle(){xhr.abort();$(".uploadifySuccess").fadeOut(1e3,function(){$(".uploadifyError").show().text("Upload canceled");});}